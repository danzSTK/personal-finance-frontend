...
export const SettingsNav = () => (
  <aside className="rounded-2xl border border-border bg-card p-3">
    <p className="hidden px-2 pb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground lg:block">
      Configurações
    </p>
    ...
    <nav className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 scrollbar-none [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden lg:mx-0 lg:block lg:space-y-2 lg:overflow-visible lg:px-0 lg:pb-0">
      {settingsNavItems.map((item) => (
        <NavLink
          ...
          className={({ isActive }) =>
            cn(
              'block min-w-42 shrink-0 rounded-xl border px-3 py-2 text-left transition focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-card lg:w-full lg:min-w-0',
              isActive
